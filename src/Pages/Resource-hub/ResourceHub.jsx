...
import listIcon from "../../assets/Icon (9).svg";
import gridIcon from "../../assets/Vector.svg";
    ...
    <div>
      <div className="relative my-5 h-14 flex justify-between px-2 items-center  text-white border border-slate-800  rounded-lg">
        ...
        <div className="flex justify-center items-center gap-6">
          ...
          <div onClick={() => setViewType("list")} className={`${viewType == "list" ? "bg-[#490FF0]" : ""} cursor-pointer border px-3 h-10 py-3 rounded-xl border-borderBottom bg-primaryBg text-white`}>
            <img className="h-full" src={gridIcon} alt="" />
          </div>
          <div onClick={() => setViewType("grid")} className={`${viewType == "grid" ? "bg-[#490FF0]" : ""} cursor-pointer border px-3 h-10 py-3 rounded-xl border-borderBottom bg-primaryBg text-white`}>
            <img className="h-full" src={listIcon} alt="" />
          </div>
        </div>
      </div>
      <div className="mt-5 w-1/2 mx-auto my-auto h-1/2">
        <Modal modal={modal} close={setModal}>
         <div className="border p-5 rounded-lg bg-[#1E2033]"> 
         <input
              placeholder="Enter Folder Name"
              className="border pl-10  text-xl  py-2 rounded-xl border-borderBottom bg-primaryBg text-white "
              type="text"
            />
          <div className="mt-8 flex p-5 justify-between items-centers text-white ">
     ...
     <div className="cardBg rounded-xl px-5">
     {
        viewType == "list" ?  
